...
yat <- round(seq(raster::extent(r)@ymin, 
                 raster::extent(r)@ymax, length.out = 3),2)
xat <- round(seq(raster::extent(r)@xmin, 
                 raster::extent(r)@xmax, length.out = 3),2)

sl1 <- list('sp.pointLabel', obs_data$xyz, label=obs_data$xyz$CODE,
            cex=0.45, col='black')


png("./data/output/plots/plot_obs_v2.png",width=900,height=700, res = 150) 

sp::spplot(r,col.regions = viridis_pal(option="D")(255),
      sp.layout=list(list(provs, col="white",fill="gray87",first=TRUE),list(border),sl1), # add it to drive
       ...
       latticeExtra::layer(sp::sp.points(obs_data$xyz,pch =19,lwd=3,col="red2"))+ 
